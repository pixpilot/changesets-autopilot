...
describe('validateOidcNodeRuntime', () => {
  it('accepts node version 22.14.0 and higher', () => {
    expect(isNodeVersionCompatibleForOidc('22.14.0')).toBe(true);
    expect(isNodeVersionCompatibleForOidc('24.0.0')).toBe(true);
  });

  it('rejects node versions lower than 22.14.0', () => {
    expect(isNodeVersionCompatibleForOidc('22.13.1')).toBe(false);
